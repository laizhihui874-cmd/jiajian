...
export function Dialog({open,title,children,onClose,wide=false}:{open:boolean;title:string;children:ReactNode;onClose:()=>void;wide?:boolean}){
 ...
 return <dialog ref={ref} className={`dialog ${wide?'wide':''}`} onCancel={onClose} onClick={e=>{if(e.target===e.currentTarget)onClose()}}><header><div><h2>{title}</h2></div><button className="icon-button" aria-label="关闭弹窗" onClick={onClose}><X size={20}/></button></header>{children}</dialog>
}
...
export async function createExportImage(design:Design,arrangement:Arrangement='original'){
 ...
 ctx.textAlign='center';ctx.fillStyle='#513E46';ctx.font='46px "Songti SC",serif';ctx.fillText(design.name,800,1440,1350);ctx.fillStyle='#8B7B7E';ctx.font='23px sans-serif';ctx.fillText('',800,1510)
...
export function downloadImage(blob:Blob,name:string){
 const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${name.replace(/[\\/:*?"<>|]/g,'-')}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),10000)
}
export async function exportDesign(design:Design,arrangement:Arrangement='original'){
 downloadImage(await createExportImage(design,arrangement),design.name)
}
