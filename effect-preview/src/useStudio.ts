...
export function useStudio(){
  const sandbox=import.meta.env.VITE_LOCAL_WORKSPACE==='1'||new URLSearchParams(window.location.search).get('demo')==='effects'
  ...
  useEffect(()=>{
    if(!ready)return
    if(sandbox){try{localStorage.setItem('nail-effects-demo-v1',JSON.stringify(design));persisted.current=fingerprint(design);setStatus('体验草稿已保留');setError('')}catch{setStatus('体验草稿未保留');setError('当前浏览器无法保留体验草稿，可保存到“我的作品”。')}return}
