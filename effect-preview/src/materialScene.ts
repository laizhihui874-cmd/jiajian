...
export function createMaterialScene(host:HTMLElement,initial:MaterialSettings,onError:(message:string)=>void,offscreen=false){
 ...
 const ornaments=new THREE.Group(),specimen=new THREE.Group();specimen.add(mesh,ornaments);group.add(specimen);scene.add(group)
 ...
 let shapeKey='oval:classic',texture:THREE.CanvasTexture|undefined,paintKey='',presentationRevision=0
 function changeShape(n:Nail,artwork=false){
  const nextShape=n.shape+':'+n.variant+(artwork?':artwork':'')
  if(nextShape===shapeKey)return
  geometry.dispose();geometry=createNailGeometry(n.shape,n.variant);mesh.geometry=geometry;shapeKey=nextShape
  if(artwork){
   // Project the complete artwork using the same frontal framing as the workspace.
   const projection=new THREE.PerspectiveCamera(32,400/640,.1,100);projection.position.z=5.9;projection.updateMatrixWorld()
   const position=geometry.getAttribute('position'),uv=geometry.getAttribute('uv'),v=new THREE.Vector3()
   for(let i=0;i<position.count;i++){v.fromBufferAttribute(position,i).project(projection);uv.setXY(i,(v.x+1)/2,(v.y+1)/2)}
   uv.needsUpdate=true
  }
 }
 function applyNailMaterial(n:Nail,light:number,paint:HTMLCanvasElement){
  const surface={...defaultSurface(),...n.surface}
  texture?.dispose();texture=new THREE.CanvasTexture(paint);texture.colorSpace=THREE.SRGBColorSpace;material.map=texture;material.needsUpdate=true
  update({...defaultMaterialSettings,color:n.color,finish:!n.artwork&&n.finish==='cat'?'cat':!n.artwork&&n.finish==='jelly'?'jelly':'gloss',angle:n.catAngle,width:surface.catWidth,strength:surface.catStrength,density:n.opacity*80,light:90})
  material.color.set('#ffffff')
  material.roughness=n.artwork?.32:n.finish==='matte'?.55+surface.shine/225:n.finish==='chrome'?.1:.06+(100-surface.shine)/250
  material.clearcoat=n.artwork?.35:n.finish==='matte'?.02:1
  material.clearcoatRoughness=n.finish==='matte'?.9:.02+(100-surface.shine)/550
  material.metalness=n.artwork?0:n.finish==='chrome'?1:n.finish==='cat'?.12:n.finish==='glitter'?.3:0
  ...
  uniforms.glitterPower.value=!n.artwork&&n.finish==='glitter'?surface.sparkle/65:0
  scene.environmentRotation.y=light/40;key.position.x=-3+light/8
 }
 function clearOrnaments(){
  ornaments.traverse(object=>{if(object instanceof THREE.Mesh){object.geometry.dispose();const materials=Array.isArray(object.material)?object.material:[object.material];materials.forEach(m=>m.dispose())}})
  ornaments.clear()
 }
 function addOrnaments(n:Nail){
  clearOrnaments()
  const projection=new THREE.PerspectiveCamera(32,400/640,.1,100);projection.position.z=5.9;projection.updateMatrixWorld()
  const shell=new THREE.Mesh(geometry);shell.updateMatrixWorld()
  ...
  for(const d of n.decorations){
   ray.setFromCamera(new THREE.Vector2(d.x/50-1,1-d.y/80),projection)
   const placementRay=ray.ray.clone()
   let hit=ray.intersectObject(shell,false)[0]
   // A partly overlapping edge ornament can have its center just outside the shell.
   // Look under its footprint rather than silently dropping that visible ornament.
   if(!hit)for(const fraction of [.4,.8,1]){
    for(let i=0;i<12&&!hit;i++){
     const angle=i*Math.PI/6
     ray.setFromCamera(new THREE.Vector2((d.x+Math.cos(angle)*d.size*fraction)/50-1,1-(d.y+Math.sin(angle)*d.size*fraction)/80),projection)
     hit=ray.intersectObject(shell,false)[0]
    }
    if(hit)break
   }
   if(!hit)continue
   const radius=d.size/100*frameWidth
   const pearl=d.type==='pearl'
   const ornament=new THREE.Mesh(pearl?new THREE.SphereGeometry(radius,24,16):new THREE.OctahedronGeometry(radius,0),new THREE.MeshPhysicalMaterial(pearl?{color:'#fff3dd',roughness:.22,metalness:.08,clearcoat:1,iridescence:.35,iridescenceIOR:1.3}:{color:'#e2efff',roughness:.08,metalness:.15,transmission:.4,thickness:radius,ior:2.1,clearcoat:1,flatShading:true}))
   const normal=hit.face?.normal.clone().normalize()||front.clone()
   ornament.position.copy(placementRay.at((hit.point.z-placementRay.origin.z)/placementRay.direction.z,new THREE.Vector3())).addScaledVector(normal,radius*.3);ornament.quaternion.setFromUnitVectors(front,normal);ornament.scale.z=pearl?.62:.5
   ornaments.add(ornament)
  }
  // Only the temporary default material is owned here; the shell geometry is shared.
  ;(shell.material as THREE.Material).dispose()
 }
 function layoutSpecimens(){
  group.clear()
  if(count===1){group.add(specimen)}else for(let i=0;i<10;i++){
   const nail=specimen.clone(true);nail.scale.multiplyScalar([.67,.62,.65,.61,.53][i%5]);nail.position.set(((i%5)-2)*1.24,(i<5?1.4:-1.4),0);group.add(nail)
  }
  resize()
 }
 ...
 return {
  update,
  async present(n:Nail,light=0){
   const revision=++presentationRevision
   let paint:HTMLCanvasElement
   if(n.artwork){
    const photo=await renderPhotoNail({...n,decorations:[]})
    if(disposed||revision!==presentationRevision)return
    paint=document.createElement('canvas');paint.width=400;paint.height=640
    const ctx=paint.getContext('2d')!;ctx.fillStyle=n.color;ctx.fillRect(0,0,400,640);ctx.drawImage(photo,0,0)
   }else paint=paintNailColor(n)
   if(disposed||revision!==presentationRevision)return
   if(lost)throw new Error('立体预览已中断，请关闭后重新打开。')
   changeShape(n,!!n.artwork);applyNailMaterial(n,light,paint);paintKey=''
   addOrnaments(n);specimen.scale.set(1,({short:.82,medium:1,long:1.18}[n.length]),1)
   scene.background=new THREE.Color('#f3eee6');layoutSpecimens();invalidate()
  },
  ...
  snapshot(n:Nail,light=0,placement?:{rotation:number;mirrored:boolean}){
   ...
   // Keep the window light in the photograph's direction after a nail is turned.
   const turn=(placement?.rotation||0)*Math.PI/180
   const lightX=(placement?.mirrored?3:-3)+light/8
   scene.environmentRotation.set(0,light/40,turn+(placement?.mirrored?-1.287:0))
   key.position.set(lightX*Math.cos(turn)-4*Math.sin(turn),lightX*Math.sin(turn)+4*Math.cos(turn),6)
  ...
  count(value:number){count=value;layoutSpecimens()},
  async download(){if(disposed||lost)throw new Error('立体预览暂时不可用，请重新打开后导出。');renderer.render(scene,camera);const blob=await new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('导出未完成，请重试');const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='甲间-立体试色.png';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)},
  dispose(){disposed=true;presentationRevision++;clearOrnaments();cancelAnimationFrame(frame);observer.disconnect();document.removeEventListener('visibilitychange',visibility);canvas.removeEventListener('pointerdown',down);canvas.removeEventListener('pointermove',move);canvas.removeEventListener('pointerup',up);canvas.removeEventListener('pointercancel',up);canvas.removeEventListener('keydown',keydown);canvas.removeEventListener('webglcontextlost',contextLost);geometry.dispose();texture?.dispose();material.dispose();environmentTarget.dispose();renderer.dispose();renderer.forceContextLoss();canvas.remove()},
 }
}
