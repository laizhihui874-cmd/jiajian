...
export async function renderNail(n:Nail,light=0){
 if(n.artwork)return renderPhotoNail(n,light)
 const quantized=Math.round(light)
 const key=JSON.stringify({...n,decorations:[],lockedLayers:[],length:'medium',light:quantized})
 ...
 if(!base){
  ...
  base=scene.snapshot(n,quantized);rendered.set(key,base)
