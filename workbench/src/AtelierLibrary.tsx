...
export function FinishedStyleControls({n,patch}:{n:Nail;patch:(p:Partial<Nail>)=>void}){
 return <section className="finished-style"><h3>{atelierStyles.find(s=>s.id===n.artwork)?.name}</h3><p>花朵、金线与光泽作为完整款式保留。可以调整长度、搭配其他甲片，也可以在“饰品”里继续添加。</p><h3>佩戴长度</h3><div className="choice-grid three">{([{id:'short',name:'短款'},{id:'medium',name:'中长'},{id:'long',name:'长款'}] as const).map(l=><button key={l.id} aria-pressed={n.length===l.id} className={n.length===l.id?'active':''} onClick={()=>patch({length:l.id})}>{l.name}</button>)}</div><button className="ghost free-design-button" onClick={()=>patch({artwork:undefined})}>换为基础甲片，自由设计</button><p className="helper">换为基础甲片后，可自行调整甲型、底色和图案；原成品图案会移除，可以撤销找回。</p></section>
}
