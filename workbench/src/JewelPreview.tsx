...
export function JewelPreview({design,selected,hand,onSelect,onDetail}:{design:Design;selected:number;hand?:'left'|'right';onSelect:(i:number)=>void;onDetail:(i:number)=>void}){
 return <div className={`jewel-layout ${hand?'jewel-five':'jewel-ten'}`} aria-label={hand?'五指珠宝陈列':'十指珠宝陈列'} role="group">
  {(hand?[hand==='left'?0:1]:[0,1]).map(side=><div className="jewel-row" key={side}><span className="jewel-hand-label">{side===0?'左手':'右手'}</span>{Array.from({length:5},(_,finger)=>{
   ...
   return <button key={i} className={`jewel-nail ${selected===i?'selected':''}`} aria-label={`编辑${nailName(i)}`} aria-pressed={selected===i} onClick={()=>onSelect(i)} onDoubleClick={()=>onDetail(i)} style={{'--nail-scale':[.94,1,1.03,.99,.86][finger],'--nail-angle':`${[-4,-2,0,2,4][finger]}deg`} as CSSProperties}>
