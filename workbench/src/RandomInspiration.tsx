...
import {generateInspiration,inspirationFamilies,inspirationMoods} from '../shared/randomInspiration'
import type {Family,GeneratedInspiration,Mood} from '../shared/randomInspiration'
...
export function RandomInspiration({design,ready,onApply}:{design:Design;ready:boolean;onApply:(d:Design,mode:'all'|'single')=>void}){
 ...
 function generate(variation:'similar'|'different'){
  ...
  const next=generateInspiration(design,{seed,mood:mood||undefined,family:family||undefined,variation,previous:current?{...current,design:preview}:undefined,keep})
 ...
 <div className="random-preferences"><label>想要的感觉<select aria-label="生成风格" value={mood} onChange={e=>setMood(e.target.value as Mood|'')}><option value="">交给你搭配</option>{inspirationMoods.map(x=><option key={x}>{x}</option>)}</select></label><label>偏好的色系<select aria-label="生成色系" value={family} onChange={e=>setFamily(e.target.value as Family|'')}><option value="">不限色系</option>{inspirationFamilies.map(x=><option key={x}>{x}</option>)}</select></label></div>
 ...
 <p className="random-help">{current?'“沿着这个感觉”保留本套配色；“换个方向”按上方偏好重新搭配。':'同色系的深浅呼应，少量重点甲，左右手有联系也有变化。'}</p>
 {current?<><div className="random-result"><strong>{current.family} · {current.mood} · {finishNames[current.finish]}</strong><div className="random-swatches" aria-label="本套配色">{current.colors.map(c=><span key={c} style={{background:c}}/>)}</div><div className="random-history"><button className="ghost" disabled={cursor<=0} onClick={()=>setCursor(cursor-1)}>上一套</button><span>{cursor+1} / {history.length}</span><button className="ghost" disabled={cursor>=history.length-1} onClick={()=>setCursor(cursor+1)}>下一套</button></div></div>
